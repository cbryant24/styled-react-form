...
import { Box, Field, Li, Div, Form, styleBuildRemove } from '@cbryant24/styled-react';
  ...
  const displayFields = () => {
    if (Array.isArray(inputs)) {
      return inputs.map( input =>  {
        ...
        return <Field key={input.data.name} data={input.data} {...field}/>
      });
    }
    const field = formInput(inputs, validate);
    return <Field data={input.data} {...field} errors={fields[inputs.data.name].errorMessages}/> 
  }
